...
-(instancetype)initWithType:(NSInteger)type{
    ...
    if (self) {
        self.type=type;
        self.frame = CGRectMake(0, 0, KScreenWidth, navigationViewHeight+pickViewViewHeight+20);
...
{
    if (self.type==2) {
          _bottomView = [[UIView alloc]initWithFrame:CGRectMake(0,0,KScreenWidth, navigationViewHeight+pickViewViewHeight)];
    }else{
    //底部父视图
    _bottomView = [[UIView alloc]initWithFrame:CGRectMake(0,KScreenHeight-280,KScreenWidth, navigationViewHeight+pickViewViewHeight)];
    }
    //_bottomView.backgroundColor=[UIColor greenColor];
    [self addSubview:_bottomView];

    //  地址选择器
    _pickView = [[UIPickerView alloc]initWithFrame:CGRectMake(0,0, KScreenWidth, pickViewViewHeight)];
    ...
    button.frame=CGRectMake(0,_pickView.frame.size.height,_bottomView.frame.size.width, navigationViewHeight);
